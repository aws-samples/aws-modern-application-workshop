...
Create a new file in the `lib` folder called `dynamodbstack.ts`.
...
Within the `dynamodbstack` file, define the following properties interface to define what constructs this stack depends upon:
...
```typescript
import ec2 = require("@aws-cdk/aws-dynamodb");
...
