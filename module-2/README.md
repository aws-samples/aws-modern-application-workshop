...
Amazon ECS has two modes: Fargate launch type and EC2 launch type. With Fargate launch type, all you have to do is package your application in containers, specify the CPU and memory requirements, define networking and IAM policies, and launch the application. For brevity in this workshop, we will use the Fargate launch type.
...
Create a new file in the `lib` folder called `networkstack.ts`.
...
new NetworkStack(app, "MythicalMysfits-Network");
```
...
Within the `NetworkStack` file, define the following VPC construct:
...
Seriously!, that is all you need to define a VPC!  Let's now run the `cdk synth` command to observe what this single line generates.  In a terminal window, navigate to `~/Workship/cdk/` and run the following command:
...
Here we are defining the CIDR block we want to assign to the VPC, the maximum number of NAT Gateways we want to establish and the maximum number of AZs we want to deploy to.
...
As before, let's create a new file within the `lib` folder, this time called `ecrstack.ts`.  And again, as before, define the skeleton structure of a CDK Stack.
...
As before, let's create a new file within the `lib` folder, this time called `ecsstack.ts`.  Define the skeleton structure of a CDK Stack.
...
then change the constructor of your EcsStack to require your properties object.
...
Notice how we reference the VPC (`props.NetworkStack.vpc`) defined previously in the NetworkStack.  AWS CDK will automatically create a reference here between the two CloudFormation objects.  Also notice that we assign the instance of the ecs.Cluster created to a local property so that it can be referenced by this and other stacks.
...
Create a new file in the `lib` folder called `cicdstack.ts`.
...
Within the `CiCdStack` file, define your complete CI/CD CodePipeline:
